  ...
  useEffect(() => {
    const fetchInstagramPosts = async () => {
      try {
        ...
        const cached = localStorage.getItem("instagram-feed");
        const cachedTimestamp = localStorage.getItem(
          "instagram-feed-timestamp"
        );

        if (cached && cachedTimestamp) {
          const lastUpdate = new Date(Number.parseInt(cachedTimestamp));
          const now = new Date();
          const timeDiff = now.getTime() - lastUpdate.getTime();

          // If cache is still valid (less than updateInterval old)
        ...
        // Cache the data
        localStorage.setItem("instagram-feed", JSON.stringify(recentPosts));
        ...
        if (cached) {
          const cachedPosts = JSON.parse(cached);
          setPosts(cachedPosts.slice(0, maxPosts));
        }
      } finally {
        setLoading(false);
      }
        ...
        <div className="flex flex-col items-center space-y-6">
          ...
          >
            <a
              ...
              href="https://instagram.com/el_pueb"
              rel="noopener noreferrer"
              target="_blank"
            >
              <Instagram className="h-5 w-5" />
              <span>Follow @el_pueb</span>
            </a>
          </motion.div>
